...
$moduleTitle = 'Widgets configuration';
...
->setTitle(EZ_TITLE . _zeT($moduleTitle) . (strpos($filter['mode'], "item") > 0 ? " - Items" . $extraTitle : ""))
...
//    $dashboard->addItem($widget);
  ...
  if ($filter['mode'] !== "") {
    switch ($filter['mode']) {
      ...
      ->addRow(_zeT('Position'), [_('Row'), '&nbsp;', (new CNumericBox('row', $data['row'], 2))->setWidth(ZBX_TEXTAREA_NUMERIC_STANDARD_WIDTH)
