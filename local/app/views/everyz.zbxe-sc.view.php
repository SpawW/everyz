...
function checkGlobalMacros() {
  ...
  /*    // Reference Average adquired in 10/2017 in research from 30 Zabbix environments */
  // Macro global do referencial em megas do armazenamento
  define("REFERENCE_GB", initiateGlobalMacro('{$EVERYZ_UBM_REF_GB}', '0.29'));
  // Macro global do referencial em VPS do processamento
  define("REFERENCE_VPS", initiateGlobalMacro('{$EVERYZ_UBM_REF_VPS}', '0.59'));
  // Minimum value for host
  define("UBM_MIN_VALUE", initiateGlobalMacro('{$UBM_MIN_VALUE}', '0.3'));
  // Reference to human custs
  define("REFERENCE_HUMAN", 0.12);
...
function totalUBM($vps, $gb) {
  ...
  $refVPS = ($vps*100/(REFERENCE_VPS*100));
  $refGB = ($gb*100/((REFERENCE_GB)*100));
  $totalUBM = round(REFERENCE_HUMAN+$refVPS+$refGB,6);
...
switch ($filter["format"]) {
  ...
  $fieldsExport = array(
    ...
    , "Storage", "VPS", "BMU", $currency, "HostID"
  );
  break;
  case PAGE_TYPE_JSON;
  $fieldsExport = array(
  ...
  ->addRow(_zeT('Not monitored items'), buttonOptions('notMonitored', (int) $filter['notMonitored'], array(_("Hide"), _("Show"))))
  ;
  $leftColumn->addItem(new CInput('hidden', 'action', $action));
  ...
  ;
  $rightColumn->addItem(new CInput('hidden', 'action', $filter["action"]));

  $dashboard->addItem(newFilterTab($moduleName,[$leftColumn,$rightColumn]));
  // Report title
  $table->setHeader(
    array($toggle_all, _("Host"), _("Item"), _("Key")
