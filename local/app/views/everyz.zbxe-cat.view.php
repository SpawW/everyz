...
function buildQueryReport($itemid, $logTable) {
  ...
  FROM " . $logTable . " hu
  WHERE hu.clock between " . strtotime($filter["from"]) . " AND  "
  . strtotime($filter["to"]) . " AND hu.itemid = " . $itemid . "
  ) a
...
if (hasRequest('filter_rst')) {
  ...
  // Put the date in required format
...
switch ($filter["format"]) {
  ...
  default: // HTML
    ...
    $leftColumn->addRow(_('Hosts'),multiSelectHosts($multiSelectHostData))
    ...
    // Build the report ------------------------------------------------------------
    $table->setHeader(array($toggle_all, (new CColHeader(_('Host')))->addStyle('width: 25%'), _zeT('Instant'), _zeT('Value'), _zeT('Type')));
    break;
}
