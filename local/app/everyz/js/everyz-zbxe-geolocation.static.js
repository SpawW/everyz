...
function addDefaultMapTiles () {
  addMapTile("OSM",'http://{s}.tile.osm.org/{z}/{x}/{y}{r}.png', '© OpenStreetMap contributors');
...
function addHost(lat, lon, hostid, popUpContent) {
  ...
  }).addTo(everyzObj.map).bindTooltip(popUpContent, {
...
function addErrorHost(lat, lon, hostid, popUpContent, severityLevel) {
  ...
  let marker = L.marker([lat, lon], {
    icon: pulsingIcon,
  }).addTo(everyzObj.map).bindPopup(popUpContent, {
    className: "zbxe-label",
    offset: [0, 0]
    , maxWidth: 500
  });
...
function editHostMetadata(hostid) {
  zbxePopUp("everyz.php?action=zbxe-geometadata&hidetitle=1&sourceHostID=" + hostid);
}
...
function hostIncidents(hostid) {
  if (zbxeZabbixVersion >= 400) {
    zbxePopUp("zabbix.php?action=problem.view&fullscreen=1&page=1&filter_show=1&filter_hostids[]=" + hostid
...
function zbxImage(iconid, width, height) {
  width = width || 32;
  height = height || 32;
  iconURL = (iconid == parseInt(iconid, 0) ? 'imgstore.php?iconid=' + iconid : "local/app/everyz/images/zpoi_" + iconid + ".png");
  return L.icon({
    iconUrl: iconURL,
...
function zbxeEditInteger (id, value, width, args) {
  ...
  return '<input id="'+id+'" name="'+id+'" value="'+value+'" style="text-align: right;width: '+
  width+'px;" onchange="validateNumericBox(this, false, true);" type="text" '+extra+'>';
}
...
function zbxeSelect (id, value, options, args) {
  ...
  for (var key in options) {
    select_options += ' <option value="'+key+'" '+(value === key ? 'selected' : '' )+'>'+options[key]+'</option>';
  }
...
function zbxeInitHostSelect(id, data, args, zbxeTranslation) {
  ...
  if (typeof zbxeTranslation === 'undefined') {
    var translation = {"No matches found":"No matches found","More matches found...":"More matches found...","type here to search":"type here to search","new":"new","Select":"Select"};
  } else {
    var translation = {"No matches found":zbxeTranslation["No matches found"],
    "More matches found...":zbxeTranslation["More matches found..."],
    "type here to search":zbxeTranslation["type here to search"],
    "new":zbxeTranslation["new"],"Select":zbxeTranslation["Select"]};
  }
  var params = {"srctbl":"hosts","dstfrm":"zbx_filter","dstfld1":"hostids_","srcfld1":"hostid","real_hosts":"1","multiselect":"1"}
  for (var key in args) {
    params[key] = args[key];
  }
  /*
...
function zbxeConfigureDivMap (mapid) {
  ...
  if (location.search.split('fullscreen=1')[1] !== undefined) {
    ...
    if (location.search.split('hidetitle=1')[1] !== undefined) {
      ...
      document.getElementsByTagName("main")[0].style.padding = "0px 0px 0px 0px";
    } else {
...
function addColorPicker(map, position) {
  ...
  legend.onAdd = function (map) {
    return newLeafletLeftBar('everyzLeftBar',divColorPicker ('000000',0)+divColorPicker(map,0,'lbl2'));
  };
  legend.addTo(map);
  return legend;
}
...
function newLeafletLeftBar(id, content) {
  ...
  div.id = id;
  div.firstChild.onmousedown = div.firstChild.ondblclick = div.firstChild.onpointerdown = L.DomEvent.stopPropagation;
  return div;
}
